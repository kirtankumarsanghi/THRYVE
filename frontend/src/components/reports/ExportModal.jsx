    ...
    try {
      ...
      if (format === 'pdf') {
        const reportEndpoint =
          exportType === 'users' ? '/admin/users' :
          exportType === 'department' ? '/analytics/departments' :
          '/reports/achievement';

        const resp = await axios.get(reportEndpoint);
        ...
        if (exportType === 'users') {
          ...
          endpoint = '/admin/export/analytics?report_type=department';
        } else {
          endpoint = '/reports/achievement/export/csv';
        }

        const resp = await axios.get(endpoint, { responseType: 'blob' });
